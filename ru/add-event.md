...
<!-- screenshot: страница события Polymarket в браузере -->
{% endstep %}
...
Откройте чат с Drops Bot в Telegram и вставьте ссылку. Бот распознает домен Polymarket и пришлёт превью события:

* Заголовок события
* Текущие котировки (Yes / No или по исходам)
* Объём за 24 часа
* Дата разрешения (если доступна)
...
Нажмите **Track Event**, чтобы добавить в вотчлист. **Cancel** — чтобы отменить.
...
Да. Мультиэйт-рынки трекаются как одно событие — алерт срабатывает при движении любого исхода.
