...
* [Часто задаваемые вопросы](#-часто-задаваемые-вопросы) — Общее · Трекинг и лимиты · Уведомления · Аккаунт и данные
* [Troubleshooting](#-troubleshooting) — таблица симптом / причина / решение
...
| Сообщение "Limit reached" | Достигнут [лимит Polymarket Events](./plan-limits.md) тарифа | Удалите ненужные оддсы через `/polymarket → 🗑` или нажмите **Build Custom Plan** на upsell-карточке |
| Оддс добавлен, но алертов нет | Слишком высокие пороги или рынок неактивен | Снизьте `📉 Price Change` / `📊 Volume Target` в ✏️ панели оддса — см. [Alerts & Filters](./alerts-and-filters.md). Проверьте, что у рынка есть недавний объём на polymarket.com |
...
| Старые события остались после удаления аккаунта | Аккаунт был удалён до cleanup-fix 9 апреля 2026 (см. [Changelog](./changelog.md)) | Обратитесь в поддержку для очистки сирот |
| Слишком много алертов | Низкие пороги для активного рынка или включён `♻️ Swaps Alert` | Поднимите `📉 Price Change` / `📊 Volume Target` per-oddsy либо выключите `♻️ Swaps Alert` |
| Изменение порога не применилось | Отредактировали не тот оддс — у мультиисходных событий по одному оддсу на исход | Откройте `/polymarket`, нажмите ✏️ на нужном оддсе, переустановите значение |
| Telegram-кэш показывает старый watchlist | Устаревшее сообщение | Отправьте `/polymarket` ещё раз для обновления |
| Разрешённые рынки всё ещё в списке | Сохраняются как история | Используйте 🗑 на каждой строке, чтобы освободить слот — см. [Лимиты тарифов](./plan-limits.md) |
