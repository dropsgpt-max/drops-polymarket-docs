...
## 💳 Сколько оддсов можно отслеживать

Квота **Polymarket Events** — название аллокации Polymarket в боте — масштабируется вместе с подпиской. Квота считает **оддсы** (один на каждый трекаемый исход, см. [Глоссарий](./polymarket.md#-глоссарий)), так что мультиисходное событие, которое вы добавили через `Select All Yes`, занимает по одному слоту на каждый исход.

Wallet-level Polymarket Activity (см. [Wallet Activity](./wallet-activity.md)) использует отдельную квоту **Wallets** и не подпадает под лимиты ниже.
